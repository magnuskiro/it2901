\section{Client User Guide}\label{Client User Guide} 
    ...
    This is the user manual for the QoS client library. Here we cover how to use the client library to communicate with our server implementation, in a way that properly set the diffserv value on the packages sent from the client to the server. This manual is intended to be as simplistic and understandable as possible.

    First we cover the steps one needs to take in order to be able to use our library. Then move on to the easiest way to use it, as well as how to use it with a listener pattern. This will be followed by a section covering the available settings in the library, and finally we bring up some caveats of our implementation that is worth keeping in mind.
